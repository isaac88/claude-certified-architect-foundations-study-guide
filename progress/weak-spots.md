...
| Instance fix vs source fix (copy a teammate's `~/.claude.json` vs repo `.mcp.json`) | 2.4 Q1 half (9 Sep); closed in 3.1 MCQ + Build 2 live; **reopened 15 Sep in schema costume** — fixed an unknown enum type by adding one more enum value instead of `other` + freeform detail | **OPEN — third costume** (contract now BUILT, not yet retested cold: exercise 3's schema carries `other` + `document_type_detail`, and its delivery-note example argues the enum is the contract every consumer already parses) | An enum/schema stem: unknown category arrives; the bait is "add it to the enum" |
| **Empty-vs-error** — in the *timeout* costume (`[]`+ok is the do-not-retry signal) and in the *code* costume (`{"customer": None}` has no `isError` → `is_error` False) | D2 practice pass; **inverted once** 13 Sep (parallel session), corrected twice same day; **wrong again 14 Sep** in Build 1's code walkthrough (said `is_error` True for a valid-empty lookup, cited a gate line) | **OPEN — four costumes**; fourth found 16 Sep in exercise 3's own harness: under a validator rule demanding an absent PO, the retry returned `''` and a `po_number is not None` test booked it as a supplied value — absence in a costume the placeholder set already knew | Turn-2 retry also times out: findings count + Coverage line, with and without partials; plus a code item: given a tool return value, what does line `is_error: bool(output.get("isError"))` produce |
...
| Haiku's silent 4096-token **caching** minimum | ex-36 Q4 FAIL, R1 micro-retest pass (7 Sep); D4 gate R5 (15 Sep) right letter, then the usage-field probe passed — `cache_creation_input_tokens` 0-always (below minimum) vs >0-every-request (TTL expired) | CLOSED | — |
| **Schema-valid is not correct** — `tool_use` + schema + `tool_choice` guarantees shape and the legal value space, and is silent about whether the value is on the page | Exercise 3 Q1 (16 Sep) **taught, not attempted**. Specimen: `oth-10` scored 9/11 in BOTH arms with a forced tool, zero validator findings — the account number promoted into `document_id`, the period end into `issue_date`, both schema-legal, both ISO-valid | **QUEUED step 28** | A record that passed schema and validation and is still wrong: name what the guarantee covers and what it is silent about |
| **Retry cannot fix an absent field** — classify every finding by who can fix it (model vs schema/rule owner) before retrying | Exercise 3 Q3 (16 Sep) **taught, not attempted**. Specimen: null → `''` under retry pressure; the cap bounds cost, the placeholder classification bounds damage | **QUEUED step 28** | A retry loop that returns a second absence in a different costume: what stops it, and what is the real fix |
| **A batch request is one turn** — the validation-retry loop is a *new* request, so batch pricing needs waves, not a cleverer request | Exercise 3 Q5 (16 Sep) **taught, not attempted**. Measured: batch 106/110 vs sync first pass 107/110 vs sync after retry 108/110; the `inv-03` conflict slot is provably the retry, the `cn-07` slot is noise on a field no validator watches | **QUEUED step 28** | Bulk extraction needs a retry loop and the CFO wants the 50%: design it, and say what latency you just bought |
...
| **Generalise** = apply the learned rule to a case not in the examples (read as "be vague"; inverted — said reasoning makes the model generalise *less*) | 4.2 Q2 FAIL (15 Sep), vocabulary + mechanism | VERIFY — **measured 16 Sep** (exercise 3): three examples with reasoning took the layouts they covered 97.0% → 100% and the layouts they did not cover 97.4% → 97.4%, so on this corpus the reasoning bought the demonstrated layouts and no others; exercise 3 Q4 (16 Sep) **taught, not attempted** — and the run cannot test 4.2's claim anyway: 3 wrong slots in 110 (no headroom) and no bare-pairs arm | Few-shot with vs without reasoning lines: what does each buy on a layout not in the examples — and what did exercise 3 measure |
| Required field on a sparse source — the schema contract makes invention the only *legal* output (nullable = absence expressible) | 4.3 Q2 half — headline without the mechanism; chain taught, mechanism then run correctly in the micro-retest (15 Sep). **Specimen built and measured 16 Sep** (exercise 3 stage 5): one blank PO box, three schemas — nullable → `None`; required non-nullable → the *string* `'null'`; nullable + a validator rule demanding a PO → `''` on retry | VERIFY — exercise 3 Q2 (16 Sep) was **taught, not attempted**, so it does not count as a pass | Blank box + required field + forced tool call: walk what the model can legally emit — and what a *retry* can legally emit once the field is still absent |
...
(a) empty-vs-error in timeout costume · (b) trim-per-consumer · (c) deterministic provenance join · (d) dedup ownership · fork direction · caching: bare "don't cache" option · caching: breakpoint placement · (e) empty-string-as-value (exercise 3's `''` costume) · (f) what a Message Batches request cannot do that the synchronous retry loop can.

## Taught, not passed — exercise 3 gap-check, 16 Sep 2026

The student asked for practice mode over exam mode and I answered all five. Recorded
here so the ledger does not read as five passes. Each gets ONE cold item at step 28, in
a costume it has not seen; two passes on different days close a row, as everywhere else.

1. What forcing the tool fixes (syntax, by construction) and what it can never fix (truth) — specimen `oth-10`.
2. Why a required non-nullable field on a blank box makes the string `'null'` the *legal* output — three constraints composing: forced `tool_choice` + `required` + no null in the type.
3. Why a retry with all three 4.4 ingredients still failed, and the classification line (not the cap) that stops it.
4. Whether the few-shot split contradicts 4.2 — no, and the run cannot test the claim: 110 slots, 3 wrong at baseline, no bare-pairs arm.
5. The batch gap as a property of what a batch request *is*, and the two-wave design that keeps both.

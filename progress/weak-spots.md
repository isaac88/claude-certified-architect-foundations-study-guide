...
| **Generalise** = apply the learned rule to a case not in the examples (read as "be vague"; inverted — said reasoning makes the model generalise *less*) | 4.2 Q2 FAIL (15 Sep), vocabulary + mechanism | VERIFY — **measured 16 Sep** (exercise 3): three examples with reasoning took the layouts they covered 97.0% → 100% and the layouts they did not cover 97.4% → 97.4%, so on this corpus the reasoning bought the demonstrated layouts and no others | Few-shot with vs without reasoning lines: what does each buy on a layout not in the examples — and what did exercise 3 measure |
| Required field on a sparse source — the schema contract makes invention the only *legal* output (nullable = absence expressible) | 4.3 Q2 half — headline without the mechanism; chain taught, mechanism then run correctly in the micro-retest (15 Sep). **Specimen built and measured 16 Sep** (exercise 3 stage 5): one blank PO box, three schemas — nullable → `None`; required non-nullable → the *string* `'null'`; nullable + a validator rule demanding a PO → `''` on retry | VERIFY | Blank box + required field + forced tool call: walk what the model can legally emit — and what a *retry* can legally emit once the field is still absent |

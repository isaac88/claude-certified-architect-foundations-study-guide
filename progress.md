...
Timed full-length mock scores (no official practice exam exists — corrected 21 Sep 2026): site 1 **900/1000** (17 Sep) · site 2 **93.33% — 56/60** (21 Sep)  
Exam date: ________
...
- [x] 28. Mixed set: **12 / 16** (target 13+) — sat 17 Sep 2026, four blocks at exam tempo, letter-only as mock-exam rehearsal, one short of target. Twelve ledger rows passed cold, including fail-open (one of the two 16 Sep cold fails) and the whole caching pair; provenance join closed outright. The four misses — fork direction (fifth round), instance-vs-source in the enum bait, where-without-why, trim-per-consumer — turned out to be one habit (behaviour patch over structural fix, ledger habit 10). **Closed by the 21 Sep cold re-sit: 3/4 with sound whys** — fork finally passed both halves, instance-vs-source and trim-per-consumer passed; where-without-why failed a third time and moves to step 30 with a 3.1/3.3 re-read
- [x] 29. Second full-length timed mock, different source — **PASSED 21 Sep 2026: 93.33% (56/60) on [certificationpractice.com](https://certificationpractice.com/practice-exams/anthropic-claude-certified-architect-foundations)** (bar 72%). The site paywalls its per-question review, so the 4 misses are unrecorded — only the score was measurable. (Corrected 21 Sep 2026: the "official Academy practice exam" this step originally named does not exist; every CCA-F practice exam is third-party — see [docs/academy-courses.md](docs/academy-courses.md).) **Both independent full-length passes achieved (90.0% and 93.3%) → book Pearson VUE**

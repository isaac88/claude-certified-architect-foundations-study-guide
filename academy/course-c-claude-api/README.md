...
| 20 | [20-handling-message-blocks.py](exercises/20-handling-message-blocks.py) | Tool replies are **multi-block**; iterate `content`, never index. History must store the **block list** — `messages.append({"role": "assistant", "content": response.content})` — and the helpers now accept blocks or text. Stops with an unanswered `tool_use` in history (steps 1–2 of the five-step flow; 3–5 are ex. 21) | **D1.1 + D1.7** — the course catches up with the repo's own divergence row and [reference/history-replay-with-tools.py](reference/history-replay-with-tools.py): flatten the assistant turn to a string and the *next* call 400s — failure displaced one turn from its origin. Divergence, live: lesson promises text + tool_use "typically"; Haiku 4.5 sent `['tool_use']` alone in both 19 and 20 — the pairing is model behaviour, not an API guarantee | _(paste section URL)_ |
| 21 | [21-sending-tool-results.py](exercises/21-sending-tool-results.py) | Steps 3–5: run the function with `**block.input`, send a `tool_result` block **in a user message** (`tool_use_id` pairs it to the request, `content` stringified, `is_error` for failures), resend full history **with `tools=` still attached** (the schema interprets history, not just new calls), receive the final text | **D1.1 + D5.3** — the measured loop: `tool_use` → `end_turn`, shape `user → assistant([tool_use]) → user([tool_result]) → assistant([text])`. The stop_reason flip IS the completion signal (step 6's answer, live). Divergence: the lesson's `response.content[1].input` assumes a text preamble at index 0 — raises `IndexError` on Haiku 4.5, where blocks were `['tool_use']`; find blocks by type, never position | _(paste section URL)_ |
| 22 | [22-implementing-multiple-turns.py](exercises/22-implementing-multiple-turns.py) | **The 1.1 agentic loop**: `while True` → chat → append blocks → `if stop_reason != "tool_use": break` → `run_tools` → repeat. Router (`run_tool`) is the extension point; `run_tools` handles N tool_use blocks per response; failures return as `is_error=True` results instead of raising | **D1.1 + D5.3** — measured a shape the notebook's own run didn't show: **two parallel `tool_use` blocks in one response** (notebook got sequential rounds; same code handles both — the model chooses per run). History: `assistant([tool_use, tool_use]) → user([tool_result, tool_result])` — step 6's "append 2 messages" answer on a live wire. Notebook's loop has **no iteration cap**: production adds a fuse and treats hitting it as failure. Ex. 04's chat bot with `input()` swapped for `run_tools()` — that swap is the whole agent | _(paste section URL)_ |
| 23 | [23-using-multiple-tools.py](exercises/23-using-multiple-tools.py) | Adding tools without touching the loop: function + schema + tools-list entry + router `elif`. Three tools serve the reminder assistant; the test request **chains** them (compute date → set reminder) | **D1.6 + D2** — exercises 22/23 are the decomposition pair: independent subtasks got batched in parallel (22), data-dependent ones forced 3 sequential rounds (23) — same code, the model derives the ordering. Also measured: the promised `['text', 'tool_use']` multi-block turn, and a D2 anti-pattern live — `set_reminder` returns `None`, the wire says `'null'`, `is_error=False`, and the model announced *"Perfect! I've set a reminder"* it could not verify. Tools must return explicit confirmations; no error ≠ worked (D5.3) | _(paste section URL)_ |
| 24 | [24-fine-grained-tool-calling.py](exercises/24-fine-grained-tool-calling.py) | Tool streaming in three stages: default mode **buffers per top-level key** (validate, then burst — measured 3.79s silence); the `fine-grained-tool-streaming-2025-05-14` beta turns validation off (steady chunks, max gap 0.81s, snapshots unparseable mid-stream by design); and **`tool_choice`** — auto / any / `{"type":"tool","name":…}` — forces a call | **D1.4 + D5.3 + D4** — `tool_choice` answers exercise 19's open question: description = guidance, `tool_choice` = enforcement (1.4's spectrum inside one request); forcing must be dropped after one round or the loop can never emit text. Stage 3 is a D5.3 specimen: the validator wrapped `"word_count": undefined` as a **string** — wrong type, `is_error=False`, failure disguised as success; only our own type check saw it. Notebook switches to `claude-sonnet-4-5` (first section off Haiku) | _(paste section URL)_ |
| 25 | [25-the-text-edit-tool.py](exercises/25-the-text-edit-tool.py) | **Anthropic-defined tool**: the request carries only a version stub (`text_editor_20250728` / `str_replace_based_edit_tool`) — Claude owns the schema, **we implement every command** (view/str_replace/create/insert) in a sandboxed `TextEditorTool` with a path-traversal gate on the model-supplied `path`. Sandbox wiped per run (gitignored) | **D2 + D5.3** — notebook bug fixed: its router matched the retired name `str_replace_editor`, so every call errored (third silent name-mismatch this course). Run 1 (our ordering bug broke backups) became a specimen: 5 consecutive `is_error` results → the model retried, adapted, then **degraded honestly** — pasted the code and admitted it couldn't edit. Run 2: 6 iterations, and the generated test suite **passes 6/6** — though the model used a million Taylor terms where Machin needs ~10: tests-pass ≠ well-engineered. Lesson's 3.5/3.7 stubs are retired; `undo_edit` gone in Claude-4+ tool versions | _(paste section URL)_ |
| 26 | [26-the-web-search-tool.py](exercises/26-the-web-search-tool.py) | The third tool species — **server tool**: their schema *and* their execution, no loop (only `pause_turn` continuations). Anatomy: `server_tool_use` (the query) → `web_search_tool_result` (hits) → text blocks with **citations** (url + `cited_text`); `max_uses` caps searches, `allowed_domains` fences sources | **D1 + D2 + D4** — measured: the `max_uses=5` **cap fired** mid-investigation (a fuse doing its job); the model refined its query 4 times server-side — a visible agentic loop that isn't ours to run; and stage 2 was a **null result**: schema present, *zero* searches — available ≠ used, so cited-answer products must force and verify (empty citations list is machine-checkable). Bonus: a cited source independently confirms exercise 06's finding that SDK 1.0 removed `temperature`/`top_p`/`top_k` | _(paste section URL)_ |
| 27 | [27-text-chunking-strategies.py](exercises/27-text-chunking-strategies.py) | Start of the **RAG block**: size-based (+overlap, the production fallback), sentence-based, structure-based (`\n## `), semantic (described only — needs embeddings). Runs all three against [report.md](exercises/report.md) — a stand-in for the course's file, with the medical-"bug-bites" vs engineering-"128-bugs" ambiguity planted for later lessons | **D5** — the chunker decides what *can* be retrieved; bad RAG answers often trace here. Strategy choice = 1.1's rule in RAG clothes: the chunker's assumptions must match the input's guarantees. Measured details a debug session stares at: mid-word cuts echoed by overlap, `re.split` consuming the `## ` markers, the pre-header preamble as chunk 0. Two latent infinite loops noted in the course's chunkers (overlap ≥ chunk size — unvalidated caller-controlled step) | _(paste section URL)_ |
| 28 | [28-text-embeddings.py](exercises/28-text-embeddings.py) | Embeddings via VoyageAI (Anthropic sells none): text → vector of floats; dimensions are learned and unlabelled — "dim 412 = oceans" is a teaching fiction. Needs `VOYAGE_API_KEY` in `.env`; `voyageai` added to requirements. `input_type` matters: `"document"` for indexed chunks, `"query"` for questions — Voyage embeds the two roles asymmetrically | **D5 + D4** — measured: a 557-char section and a 9-word question both map to 1024 dims — the shared space IS the mechanism; values sit in ±0.11, not ±1 (bound ≠ magnitude). Chunks embed at index time, queries per request — only the query is a marginal cost. Changing embedding model silently re-ranks the whole corpus: re-run the retrieval eval after. Install note: the client pulls ~35 packages incl. numpy/langchain-core | _(paste section URL)_ |
| 29 | [29-implementing-the-rag-flow.py](exercises/29-implementing-the-rag-flow.py) | The five retrieval steps by hand: chunk → batch-embed (`"document"`) → store vector+text pairs in [vector_index.py](exercises/vector_index.py) (course scaffolding, brute-force cosine) → embed question (`"query"`) → smallest-distance k. Steps 1–3 are index-time, 4–5 per question; no Claude call yet | **D5 + D4** — the exercise-27 trap sprang right: "how many bugs did engineers fix" ranked engineering (0.39) over medical bug-bites (0.55) — meaning beat string match, wide gap. Retrieval is a checkable stage: right-chunk-per-known-question is an eval that needs no Claude. Rank is robust, absolute distance is not (course's own run: 0.71 vs 0.72). Fifth course-code bug: notebook calls `store.add_documents` — a method its class doesn't define | _(paste section URL)_ |
| 30 | [30-bm25-lexical-search.py](exercises/30-bm25-lexical-search.py) | Lexical search via [bm25_index.py](exercises/bm25_index.py): IDF-weighted exact-token overlap (rare terms dominate, k1 damps repetition, b damps long docs). Same doc shape and lower-is-better scores as `VectorIndex` — interface unification for the coming hybrid Retriever. BM25 is local: no API, no key, no per-query cost | **D5 + D4** — measured honestly: on 5 chunks **both** indexes answered both query classes correctly; the argument lives in margins (semantic matched the incident ID at its weakest confidence — context, not identifier) and accidents (BM25 dodged the bug-bites trap only because unstemmed "bugs" ≠ "bug"). Toy corpora demo mechanics, not failure rates — retrieval evals on real data decide. Also: `vector_index.py` gained `add_documents` — the course itself fixed ex. 29's missing-method bug one notebook later | _(paste section URL)_ |
| 31 | [31-a-multi-index-rag-pipeline.py](exercises/31-a-multi-index-rag-pipeline.py) | RAG block close: [retriever.py](exercises/retriever.py) wraps both indexes behind one `SearchIndex` protocol and fuses **rank positions** with RRF (`Σ 1/(k_rrf + rank)`) — raw scores are incommensurable, first-place is not. Same composition move as ex. 22's tool router | **D5 + D2** — measured: engineering topped both indexes on both queries (fused 0.03279 = the k_rrf=60 maximum); preamble and medical **tied exactly** on mirrored ranks (2nd/3rd vs 3rd/2nd) — RRF cancels symmetric disagreement. Plus a two-run rate-limit saga: Voyage free tier is 3 req/min **per account over a trailing minute** — a per-process throttle can't see the previous run's calls; fixes were cache + throttle at the shared boundary, not retries. Gotchas recorded: RRF is higher-is-better (its inputs are lower-is-better); fusion identity is `id(doc)` — serialise the corpus and merging silently stops | _(paste section URL)_ |

| 32 | [32-extended-thinking.py](exercises/32-extended-thinking.py) | Start of the **Features of Claude block** — extended thinking: `thinking={"type":"enabled","budget_tokens":N}` buys a `thinking` block (with tamper-proof `signature`) before the `text` block; budget min 1024, `max_tokens` must exceed it; enable only when evals say the optimised prompt still misses | **D4 + D5** — measured: same question, 402 → 1797 output tokens and 4.1s → 16.1s with thinking on (billed as output; budget is a ceiling). The magic string DID force a `redacted_thinking` block while the model's visible text denied the string does anything — the wire contradicts the self-report (D5.3). Sixth course-code bug: page's `"budget"` key 400s, real param is `budget_tokens` | [287773](https://anthropic.skilljar.com/claude-with-the-anthropic-api/287773) |
...
| `thinking = {"type": "enabled", "budget": N}` | `budget_tokens`, not `budget` — on **pre-4.6 models only**. On 4.6+/5 families the budget concept is gone: thinking is adaptive (`{"type": "adaptive"}`) and `budget_tokens` itself is a 400; depth is tuned with `output_config.effort` | Tested live 3 Sep 2026 on `claude-haiku-4-5`: the lesson's key returns `400 "thinking.enabled.budget_tokens: Field required"`. Likely a page transcription error (exercise 32) |
